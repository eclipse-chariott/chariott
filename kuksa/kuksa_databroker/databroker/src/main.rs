...
use databroker::{broker, chariott_provider, grpc, vss};

// Hardcoded datapoints
const DATAPOINTS: &[(&str, broker::DataType, broker::ChangeType, broker::EntryType, &str)] = &[
    (
...
// use chariott_common::error::ResultExt as _;
// examples_common::chariott::provider::main!(wain);
//
// async fn wain() -> Result<(), chariott_common::error::Error> {
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let addr = format!("{}:{}", args.value_of("address").unwrap(), args.value_of("port").unwrap());
    ...
    if let Some(filename) = args.value_of("metadata") {
        ...
        let entries = vss::parse_vss(&data)/*.map_err_with("error parsing VSS")*/?;
            ...
            if let (Ok(id), Some(default)) = (id, entry.default) {
                ...
                if let Err(errors) = broker.update_entries(ids).await {
                    ...
                    if let Some(error) = errors.get(0) {
                        info!("Failed to set default value for {}: {:?}", entry.name, error.1);
                    }
                }
            }
        }
    ...
    grpc::server::serve_with_shutdown(
        &addr,
        broker,
        chariott_provider::Provider::new(url),
        shutdown_handler(),
    )
    .await
    .unwrap();
